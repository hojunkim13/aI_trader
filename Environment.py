...
class Environment:
    ...
    def step(self, order, render = False):
        buy = self.unnormal(self.data['Open'][self.idx])
        sell = self.unnormal(self.data['Close'][self.idx])
        ...
        if render:
            ...
            print(time, f' [주문(today): {order:.2f}] [일일 수익률: {(profit*100):.1f}%] [수익: {income:.0f} 원]')
